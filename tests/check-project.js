...
for(const file of ['V1.0.9_实现差异与开发说明.md','V1.0.9_测试报告.md','V1.0.9_回归报告.md','V1.0.9_发布报告.md','V1.0.9_待确认事项.md']) assert.ok(fs.existsSync(path.join(root,file)),file);
for(const file of ['V1.0.10_实现与差异说明.md','V1.0.10_测试报告.md','V1.0.10_回归报告.md','V1.0.10_发布报告.md','V1.0.10_待确认事项.md']) assert.ok(fs.existsSync(path.join(root,file)),file);
...
assert.match(html,/PWA V1\.0\.12/);
assert.match(html,/id="derivedSharesCard"/);
assert.match(html,/id="simulatedValueCard"/);
assert.match(html,/id="page-share-timeline"/);
assert.match(html,/id="page-simulation-basis"/);
assert.match(html,/id="btnNewCalibration"/);
assert.match(html,/自动模拟市值并非基金账户真实余额/);
...
assert.match(html,/id="mSimulatedTotalAssets"/);
assert.match(html,/id="mTotalAssetsStatus"/);
assert.match(html,/id="aSimulatedAssetsStatus"/);
assert.match(html,/此明细不替代基金公司真实成交确认流水/);
assert.match(html,/await this\.renderShareTimeline\(\);await this\.renderSimulationBasis\(\);await this\.renderFundBasis\(\);await this\.renderAnalysis\(\)/);
assert.match(html,/当前递推份额/);
assert.match(html,/正式XIRR/);
assert.match(html,/模拟XIRR/);
assert.match(html,/id="mPeriodReturn"/);
assert.match(html,/接管期较短，XIRR年化波动可能显著放大，仅供参考/);
assert.match(html,/openPendingRecords/);
assert.match(html,/确认已执行/);
assert.match(html,/id="realtimePoint"/);
assert.match(html,/id="marketClose"/);
assert.match(html,/实时行情仅供市场观察/);
assert.match(readme,/PWA V1\.0\.12/);
assert.match(githubReadme,/V1\.0\.12/);
...
assert.match(html,/schemaVersion:4,appVersion:'1\.0\.12',databaseVersion:5/);
...
assert.match(sw,/csi1000-pwa-v1\.0\.12/);
