# 中证1000策略助手 V1.0.10｜GitHub Pages + GitHub Actions 版
...
V1.0.9 不升级数据库结构，新增待执行记录筛选与确认闭环、接管后区间收益率及短接管期XIRR提示；既有V1.0.8数据可直接读取。

V1.0.10 不升级数据库结构，新增递推份额变化明细、校准历史快捷入口和模拟市值依据详情；全部结果继续从既有事实动态重建，既有V1.0.9数据可直接读取。
