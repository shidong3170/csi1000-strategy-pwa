(function(root,factory){
  ...
  function calcDerivedShares(calibrations,cycles,trades,asOfDate='9999-12-31'){
    ...
    for(const record of (trades||[]).filter(x=>x.type==='MANUAL_REDEEM'))if(isConfirmed(record)&&recordDate(record)<=asOfDate&&afterAnchor(record,anchor))redeems+=BigInt(asSafeNumber(record.redeemedSharesMicro||0,'REDEEMED_SHARES'));
  ...
  function buildShareTimeline(calibrations,cycles,trades,shareEvents=[],revisions=[],asOfDate='9999-12-31'){
    ...
    const addBuy=(record,entityType)=>{
      ...
      if(!audits.length){candidates.push({eventType:record.shareConfirmationStatus===SHARE_STATUS.MANUAL_CORRECTED?'MANUAL_SHARE_CORRECTION':'BUY_CONFIRMED',eventName:record.shareConfirmationStatus===SHARE_STATUS.MANUAL_CORRECTED?'人工份额纠偏':'买入份额确认',date:recordDate(record),occurredAt:record.shareConfirmedAt||record.createdAt||recordDate(record),deltaSharesMicro:current,sourceType:entityType,sourceId:record.id,shareStatus:record.shareConfirmationStatus,shareSource:record.shareSource||'RECORD',amountCent:Number(record.actualAmountCent??record.amountCent)||0,unitNavScaled:record.unitNavScaled??null,reconstructable:true});return}
      let previous=0;
      for(let index=0;index<audits.length;index++){
        ...
        candidates.push({eventType:revision?'NAV_REVISION_RECALC':'BUY_CONFIRMED',eventName:revision?'净值修正重算':'买入份额确认',date:audit.tradeDate||recordDate(record),occurredAt:audit.confirmedAt||record.shareConfirmedAt||record.createdAt||recordDate(record),deltaSharesMicro:value-previous,sourceType:entityType,sourceId:record.id,shareStatus:revision?SHARE_STATUS.MANUAL_CORRECTED:SHARE_STATUS.CONFIRMED,shareSource:audit.source||record.shareSource||'RECORD',amountCent:audit.actualAmountCent??(Number(record.actualAmountCent??record.amountCent)||0),unitNavScaled:audit.unitNavScaled??record.unitNavScaled??null,reconstructable:true});previous=value;
      }
      if(previous!==current)candidates.push({eventType:'MANUAL_SHARE_CORRECTION',eventName:'人工份额纠偏',date:recordDate(record),occurredAt:record.shareConfirmedAt||record.modifiedAt||record.createdAt||recordDate(record),deltaSharesMicro:current-previous,sourceType:entityType,sourceId:record.id,shareStatus:record.shareConfirmationStatus,shareSource:record.shareSource||'RECORD',amountCent:Number(record.actualAmountCent??record.amountCent)||0,unitNavScaled:record.unitNavScaled??null,reconstructable:true});
    };
    ...
    for(const record of (trades||[]).filter(x=>x.type==='MANUAL_REDEEM'))if(isConfirmed(record)&&recordDate(record)<=asOfDate&&afterAnchor(record,anchor)){const value=asSafeNumber(record.redeemedSharesMicro||0,'REDEEMED_SHARES');candidates.push({eventType:'REDEEM_CONFIRMED',eventName:'赎回份额确认',date:recordDate(record),occurredAt:record.shareConfirmedAt||record.createdAt||recordDate(record),deltaSharesMicro:-value,sourceType:'manual_trade',sourceId:record.id,shareStatus:record.shareConfirmationStatus,shareSource:record.shareSource||'RECORD',amountCent:Number(record.redeemAmountCent??record.amountCent)||0,unitNavScaled:record.unitNavScaled??null,reconstructable:true})}
    ...
    const historyComplete=running===derived.derivedSharesMicro;
    if(!historyComplete){events.push({eventType:'NAV_REVISION_RECALC',eventName:'当前事实重算',date:asOfDate,occurredAt:asOfDate,beforeSharesMicro:running,deltaSharesMicro:derived.derivedSharesMicro-running,afterSharesMicro:derived.derivedSharesMicro,sourceType:'derived_facts',sourceId:null,amountCent:null,unitNavScaled:null,reconstructable:false});running=derived.derivedSharesMicro}
    return {status:SIMULATION_STATUS.AVAILABLE,anchor,currentSharesMicro:running,pendingCount,events,historyComplete,revisionCount:(revisions||[]).filter(x=>['fund_nav','investment_cycle','manual_trade'].includes(x.entityType)).length};
  }
